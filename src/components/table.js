...
root.container.addEventListener('reset', () => {
    // 1. Сбрасываем значения всех фильтров в объекте state
    // Обычно это делается путем присвоения пустого объекта или вызова метода сброса
    state.filters = {}; 

    // 2. Используем setTimeout, чтобы дождаться, пока браузер очистит DOM-поля
    setTimeout(() => {
